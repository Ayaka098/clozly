...
- 未ログイン時のヘッダーログインボタンをGoogleログインへ直接遷移する動作に変更
- ヘッダーロゴのリンク先を`/search`から`/`に変更
- 基本情報の保存/画像削除ボタンを同サイズに調整
- ログアウト時に確認ダイアログを追加
- 画像削除ボタンに淡い色を付けて背景との差を確保
- 画像削除ボタンを横長寄りのサイズに調整
- 画像削除ボタンを小さめの横長形状に再調整
- ログインボタンのGoogle認証を`callbackUrl: "/"`＋`prompt=select_account`で直接遷移するよう調整（ログアウト後のOAuthSignin回避）
- ログイン後の遷移先を`/account`に変更し、結果ページの「戻る」「試着」ボタンの視認性を強化
- マイページの画像削除ボタンをカード幅いっぱいに拡張し、高さを小さめに調整
- マイページの画像削除ボタンの高さを半分程度に調整（横幅は従来どおり）
- 画像削除ボタンの横幅はカード幅いっぱいを維持
- 検索結果カードに「この服を試着」ボタンを追加し、押下でプロンプト自動生成→`/tryon`へ遷移
- 同一画像/URLの候補が並ばないよう、選定時に画像URL正規化＋重複除外を追加（多様性を強化）
- 画像URLの正規化を強化し、同名/同ブランドの重複候補も除外して4件の多様性をさらに確保
- マイページの画像削除ボタンの高さを2.5倍に調整
- 試着ページのプロンプト/全身画像を横並びにし、画像サイズを抑えてボタンをタイトル横へ移動
- 試着ページのプロンプト枠と全身画像枠の高さを揃える調整を追加
- 試着ページの「画像をコピー」ボタンの背景色を変更して視認性を改善
- 試着ページのプロンプト枠にGeminiへのリンクボタンを追加
- Geminiリンクボタンの視認性を上げ、アンダーラインを除去
- プロンプト/画像コピー時のメッセージを各枠内に表示し、2秒で消えるように変更
- 「最新結果」タブとページを追加し、直近の検索結果に戻れる導線を用意
- タブ順を「検索 / 検索結果 / 試着 / タイプ診断」に並べ替え、タイトルも合わせて更新
- 「全身画像」表記を「人物画像」に変更
- 試着プロンプトに商品名/説明/画像URL/商品URLなどを追加し、商品画像・商品ページへのリンクも表示
- 試着ページで商品画像を開く代わりに商品画像をコピーできるように変更
- 試着ページのリンク系ボタンを少し小さくし、人物画像コピーをプロンプトコピーと同色に統一
- 人物画像コピーのボタンを画像の下に移動して表示
- 人物画像の内側枠（画像＋コピー按钮をまとめた枠）を削除
- 商品画像コピーが失敗した場合は画像URLをコピーするフォールバックを追加
- 商品画像コピーを安定させるため、画像プロキシAPIを追加してブラウザで直接コピー可能に
- 試着選択時に商品画像のプロキシURLを保存し、コピー処理は必ずプロキシ経由で行うよう統一
- 既存セッションでもプロキシ経由になるよう、試着ページでURLを強制プロキシ変換
- 商品画像コピー時にPNGへ変換してクリップボード互換性を向上
- 試着ページのプロンプト入力欄を削除し、Gemini合成の手順説明に差し替え
- 合成手順の説明文を文言変更し、黄色枠を廃して文字を小さく調整
- マイページの保存メッセージをポップアップではなく保存ボタン横のグレー表示に変更
- 試着ページの「画像をコピー」ボタンを全身画像の枠内に配置
- スマホ時のヘッダーを1行寄りに整え、ロゴ/タブ/ログインのサイズを縮小してタブの縦並びを防止
- スマホ時のタブを横スクロール前提に変更し、タブ高さを抑えて見た目をPC寄りに安定化
- NextAuthに`trustHost: true`を追加し、デプロイ環境でのホスト検証エラーを回避（NextAuth v4の型エラーのため撤回）
- 未ログイン時の`/account`でGoogleログインボタンを表示しないように変更
- 試着プロンプトに「人物画像・商品画像をアップロード」「Nano Bananaを使う」指示を追加
- 試着プロンプトに人物画像（全身）と商品画像（服）の区別を明記する文を追加
- 試着プロンプトに全身生成（頭から足先まで）と足元が切れない構図の指示を追加
- 人物画像が全身でない場合でも全身生成する指示と、人物/商品画像の貼り順（人物→商品）を追記
- 人物/商品画像の順序案内文言を「人物画像→商品画像」に統一
- 試着ページの商品画像コピー/商品ページボタンの背景色を少し薄く調整
- 試着ページのボタン色が反映されるよう、上位セレクタで商品系ボタンの色指定を強化
- 検索条件をセッションに保存して再訪時も維持、条件リセットボタンを追加
- 試着プロンプトに「人物画像のポーズ/体型のみ参照」「商品画像の人物は無視」指示を追加
- /resultsページの試着ボタンを削除し、候補なし時の文言を「見つかりませんでした...」に変更（カード内の試着ボタンは復活）
- 試着ページの「商品画像をコピー」左に「人物画像をコピー」ボタンを追加
- タイプ診断ページをMVP向けに「Coming Soon...」の中央表示へ置き換え
- タイプ診断の「Coming Soon...」にスロー表示と柔らかな発光演出を追加
...
- 「スマホ表示のタブが太すぎるためPC寄りの配置に整え、デプロイ後のログインエラーも修正してほしい。」
- 「/accountの未ログイン時に表示されるログインボタンを削除してほしい。」
- 「試着プロンプトに人物/商品画像アップロード前提とNano Banana指示を追加してほしい。」
- 「人物画像と商品画像が混同されないよう、区別を明記した文にしてほしい。」
- 「足元だけにならないよう全身生成を指示し、人物画像が全身でない場合の扱いもプロンプトに反映してほしい。」
- 「人物画像→商品画像の順でアップロードする文言に変更してほしい。」
- 「検索タブの条件を離れても保持し、リセットボタンで初期化できるようにしてほしい。」
- 「商品画像の人物に引っ張られるため、プロンプトで人物参照ルールを強化してほしい。」
- 「/resultsの試着ボタンを削除し、候補なし時の文言を指定の内容に変更してほしい。」
- 「/resultsの試着ボタンは各カードのみ復活してほしい。」
